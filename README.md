...
- ⏰ **Built-in scheduler** (APScheduler, in-process) — either one daily run, or a decoupled **collect (every few hours) → live "Today" draft → evening delivery** pipeline so the portal shows the day as it builds up while email/Telegram still goes out once a day. The daily send fires at your configured time in a **configurable timezone** (not the container's UTC clock). The collect/draft intervals are **anchored to local midnight** (so restarts don't reset the clock) and **jittered ±20 min** so the scrape cadence looks human. The evening delivery **waits for any in-flight scrape** to finish rather than skipping itself, so a collision with the collect schedule can't drop a day's send. The collector early-stops on already-seen tweets so frequent scraping stays light. CLI: `ingest` / `process` / `deliver`.
- 📜 **Activity log** — an **Activity** page logging every background-schedule cycle — both **collection** (scrape) and **processing** (live-draft refresh) — with status, trigger (schedule vs manual), and counts, plus the **next run time** for each. Includes idle cycles that found nothing new, so the real cadence is visible. Each draft refresh links to the **interim digest it rendered**, so you can revisit the digest as it looked at any point in the day — not just the current draft.
